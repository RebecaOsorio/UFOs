...
function pickValidData(){
  let dataKeys = Object.keys(tableData);
  let randomKey = dataKeys[Math.floor(Math.random()*dataKeys.length)];
  return validRow = tableData[randomKey];
}
...
function updateFilters() {
  ...
  function filterTable() {
    ...
    if(filteredData.length === 0){
      let exampleData = pickValidData()
      filteredData = tableData;
      console.log("Parameters not valid. Try with a different combination of parameters");
      console.log("Date: "+exampleData.datetime)
      console.log("State: "+exampleData.state)
    };
